...
allprojects {
  ...
  tasks.matching { it.name == "check" }.configureEach { dependsOn(tasks.named("ktfmtCheck")) }
}
...
tasks.register<Copy>("releaseArtifacts") {
  ...
  into("jar") {
    from(project(":main").tasks.named("jar")) { rename { "francis.jar" } }
    from(project(":demo").tasks.named("jar")) { rename { "francis-demo.jar" } }
    from(project(":hostSdk").tasks.named("jar")) { rename { "francis-host-sdk.jar" } }
  }

  // Copy SDK jar to sdk/ subdirectory for Maven publishing
  into("sdk") { from(project(":hostSdk").tasks.named("jar")) { rename { "francis-host-sdk.jar" } } }
