...
internal fun buildInstrumentationArgsList(
  ...
  val instrumentationArgs: Map<String, String?> =
    runnerVals.instrumentationArgs +
      mapOf(
        ...
        // Preserve the host-resolved app package even when it differs from
        ...
        "androidx.benchmark.suppressErrors" to
          (if (runnerVals.suppressErrors) "LOW-BATTERY,DEBUGGABLE,EMULATOR" else ""),
  ...
  return instrumentationArgs.flatMap { (k, v) ->
    if (v != null) listOf("-e", k, v) else emptyList()
  }
}

class Benchmark(val baseVals: BaseValues, val runnerVals: RunnerValues) {
  ...
  val deviceOutputDir: String by lazy {
    "/sdcard/Android/media/$instrumentationPackage/additional_test_output"
  }
  ...
  private val isRootAvailable: Boolean by lazy {
    val result =
      adb.shellStdout("su", "0", "id", allowedExitCodes = listOf(0, 1, 255)) {
  ...
  fun run() {
    ...
    simpleperfOutputDir?.let {
      ...
      adb.shellRun("mkdir", "-p", it, forceRoot = isRootAvailable) {
        logPriority = LogPriority.DEBUG
      }
    }
    ...
    val cmdArgs =
      arrayOf("am", "instrument", "-w") +
        instrumentationArgsList +
        arrayOf("$instrumentationPackage/${runnerVals.runnerClass}")

    // Show logcat for the instrumentation process (UiDevice is absurdly verbose, so we silence
    // that)
    val packageLogcatHelper =
      PackageLogcatHelper.create(instrumentationPackage, filterSpecs = listOf("UiDevice:e", "*:v"))

    val instrumentProc =
      adb.shellStart(*cmdArgs) {
        stdoutRedirect =
          OutputRedirectSpec(listOf(OutputTarget.Capture)) +
            loggedStdoutRedirectSpec(LogPriority.INFO)
        stderrRedirect =
          OutputRedirectSpec(listOf(OutputTarget.Capture)) +
            loggedStderrRedirectSpec(LogPriority.INFO)
      }
  ...
  /**
   * Pull all files from a device directory to a host directory. We pull the entire directory at
   * once to avoid race conditions with temp files that may be deleted between listing and pulling.
   */
  ...
  private fun ensureInstalled(apk: String) {
    val sha256 =
      subproc
        .stdout("sha256sum", apk) { logPriority = LogPriority.DEBUG }
        .split("\\s+".toRegex())
        .first()
