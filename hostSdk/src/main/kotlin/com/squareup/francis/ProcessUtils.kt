...
class Adb(private val subproc: SubProc, val serial: String = detectSerial(subproc)) {
  ...
  fun cmdStart(vararg args: String, configure: TeeProcessBuilder.() -> Unit = {}): TeeProcess {
  ...
  private fun buildShellCommand(
    ...
    val procArgs =
      listOf("sh", "-c", """adb -s $serial shell $(printf " %q" "$@")""", "--") +
        maybeRoot +
        listOf("sh", "-c") +
        maybeEval +
        listOf("--") +
