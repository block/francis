...
class FrancisBenchmarkRule : TestRule {
    ...
    when (FrancisConfig.profiler) {
      ...
      "simpleperf" -> {
        effectiveMeasureBlock = {
          SimpleperfProfiler(
              requireNotNull(FrancisConfig.simpleperfOutputDir) {
                "${FrancisConfig.SIMPLEPERF_OUTPUT_DIR_ARG} not set"
              },
              testName,
              packageName,
  ...
  private fun createPerfettoConfig(packageName: String): PerfettoConfig {
    val configText =
      FrancisConfig.perfettoConfigPath?.let { path -> java.io.File(path).readText() }
