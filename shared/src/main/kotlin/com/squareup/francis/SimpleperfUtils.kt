 ...
 *
 * Some devices don't support hardware cpu-cycles events (e.g., emulators), so we fall back to
 * cpu-clock.
 */
object SimpleperfUtils {
   ...
   * @param supportsCpuCycles Whether the device supports cpu-cycles events (use [supportsCpuCycles]
   *   to check)
   ...
   * @param useRoot Whether to prefix the command with `su 0` for root access (required for
   *   system-wide profiling)
   * @return List of command arguments starting with "simpleperf" (or "su 0 simpleperf" if useRoot
   *   is true)
   */
